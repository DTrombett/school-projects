...
const orarioDocenti = await fetch("_data/orarioDocentiArtistico.json").then(
	(b) => b.json()
);
/** I nomi dei docenti */
	...
	if (event.target instanceof HTMLLIElement) {
		...
		for (let i = 1; i < table.rows.length; i++) {
			...
			for (let j = 1; j < table.rows[i].cells.length; j++) {
				...
				className = `${className[0]} ${className.slice(1)}`;
				...
				table.rows[i].cells[j].style.backgroundColor = `rgba(${
					className === "Ricevimenti"
						...
						: `${CLASS_NUMBER_STEP * +className[0]}, ${
								CLASS_COURSE_STEP * 1.5
						  }, ${
								CLASS_NAME_STEP *
								(className.charCodeAt(2) - FIRST_CHAR_CODE + 1)
						  }`
				}, 0.75)`;
